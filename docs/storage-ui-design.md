...
**Navigation.** One new entry added to `shellNav.ts`'s `nav-administration` section, alongside the existing `catalog-management` entry: `{ id: 'storage-tiers', label: t('Storage tiers'), path: '/admin/storage-tiers' }` [Codebase: `apps/app-frontend/src/shell/shellNav.ts`]. Routed in `AppShell.tsx` as `path="/admin/storage-tiers/*"` to `StorageTiersListPage`, following the same wiring as `/admin/catalog/*`.
